...
legend_D1  <- make_legend("1er Décile revenu →", "Tercile")
legend_D9 <- make_legend("9e Décile revenu →", "Tercile")
legend_D91 <- make_legend("Rapport interdécile D9/D1 →", "Tercile")
legend_M <- make_legend("Revenu Médian →", "Tercile")

polluants_label <- c("NO<sub>2</sub>","O<sub>3</sub>","PM<sub>2.5</sub>","PM<sub>10</sub>")
polluants <- c("NO2","O3","PM25","PM10")
rev_levels <- c("Q221")
rev_label <- c(Q221 = "Médiane")

edu_pol  <- as.vector(outer(polluants_cl, edu_cl, paste, sep = "_"))
immi_pol <- as.vector(outer(polluants_cl, immi_cl, paste, sep = "_"))

titles <- expand.grid(pol = polluants, rev = rev_levels) %>%
  mutate(
    var   = paste0(pol,"_", rev),
    titre = paste0(
      "Exposition au ", polluants_label[pol],
      " et ",
      rev_label[rev], " de revenu"
    )
  )
...
for(i in seq_len(nrow(titles))){
  ...
  legend_use <- {
    if (grepl("D1", titles$var[i])) legend_D1
    else if (grepl("Med", titles$var[i])) legend_M
    else legend_D9
  }
  ...
  ggsave(
    paste0("2_results/Map/Bivariate/Tercile/Revenu/map_", titles$var[i], "_2021.png"),
    final_plot,
    width = 5, height = 5,
    bg = "#fbf9f4", dpi = 300
  )
...
for(i in seq_len(nrow(titles))){
  ...
  ggsave(
    paste0("2_results/Map/Bivariate/Seuil/Revenu/map_", titles$var[i], "_2021.png"),
    final_plot,
    width = 5, height = 5,
    bg = "#fbf9f4", dpi = 300
  )
