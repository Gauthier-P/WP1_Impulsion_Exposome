...
map_data_2024 <-  readRDS("0_input/map_data_COG2024_C_2021_biv2000.rds")
...
make_legend <- function(label){
  expand.grid(Pollut = 1:3, Edu = 1:3) %>%
    ...
    labs(x = "Tercile polluant →", y = label) +
...
legend_immi  <- make_legend("Tercile immigration →")
...
titles <- expand.grid(pol = polluants, immi = immi_var) %>%
  mutate(
    ...
    titre = paste0(
      ...
      " et proportion d'individus immigrés | Communes >2000 hab."
    )
  )
...
for(i in seq_len(nrow(titles))){
  ...
  base_plot <- plot_bivar(map_data_2024, titles$var[i], titles$titre[i])
  ...
  ggsave(
    paste0("2_results/Map/Bivariate/Immi/map_", titles$var[i], "2000_2021.png"),
    final_plot,
    width = 5, height = 5,
    bg = "#fbf9f4", dpi = 300
  )
