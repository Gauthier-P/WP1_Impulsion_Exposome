...
wb <- createWorkbook()
Variable <- c(Polluants,Seuil,  "High_edu_prop","Prop_Immigre", "Q221")
label.Variable <- c("NO2", "O3", "PM2.5", "PM10", 
                    Seuil.Label,
                    " Prop. niveau élevé éducation",
                    "Prop. d'individus immigrés", "Médiane")
data <- map_data
...
saveWorkbook(wb, "2_Results/Description/table_comp_raw.xlsx", overwrite = T)
